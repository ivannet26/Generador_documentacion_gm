...
def ensure_solicitudes_schema(conn):
    ...
    columnas = {
        "sheet_uid": "TEXT", "marca_temporal": "TEXT", "marca_dt": "TEXT",
        "correo_solicitante": "TEXT", "correo": "TEXT", "tipo_documento": "TEXT",
        "nombres": "TEXT", "apellidos": "TEXT", "documento": "TEXT",
        "fecha_inicio": "TEXT", "fecha_fin": "TEXT", "universidad": "TEXT",
        "codigo_alumno": "TEXT", "facultad": "TEXT", "carrera": "TEXT",
        "ciclo": "TEXT", "cargo": "TEXT", "actividades": "TEXT",
        "horas_totales": "INTEGER", "estado": "TEXT", "observaciones": "TEXT",
        "fecha_revision": "TEXT", "revisado_por": "TEXT", "fecha_emision": "TEXT",
        "emitido_por": "TEXT", "codigo_documento": "TEXT", "ruta_pdf": "TEXT",
...
def ensure_reportes_schema():
    try:
        ...
        print(f"Error ensure_reportes_schema: {repr(e)}")
...
def solicitudes_detalle(sid):
    conn = db()
    sol = get_solicitud_por_id(conn, sid)
    conn.close()
    if not sol:
        flash("Solicitud no encontrada", "error")
        return redirect(url_for("solicitudes"))
    historial = get_historial(sid)
    return render_template("detalle.html", active="solicitudes", s=sol, historial=historial)
...
def solicitudes_guardar(sid):
    ...
    if not sol:
        ...
        return redirect(url_for("solicitudes"))
    cambios = []
    if (sol["horas_totales"] or "") != horas:
        cambios.append(f"Horas {sol['horas_totales'] or ''} -> {horas}")
    if (sol["observaciones"] or "") != observ:
    ...
    conn.close()
    if cambios:
        add_historial(sid, sol["documento"] or "", session.get("nombre", "USUARIO"), "GUARDAR", " | ".join(cambios), f"{sol['nombres']} {sol['apellidos']}", sol["tipo_documento"])
...
def solicitudes_marcar_observado(sid):
    ...
    conn.commit()
    conn.close()
    add_historial(sid, sol["documento"] or "", session.get("nombre", "USUARIO"), "OBSERVADO", "Marcado como OBSERVADO", f"{sol['nombres']} {sol['apellidos']}", sol["tipo_documento"])
...
def solicitudes_marcar_revisado(sid):
    ...
    conn.commit()
    conn.close()
    add_historial(sid, sol["documento"] or "", session.get("nombre", "USUARIO"), "REVISADO", "Marcado como REVISADO", f"{sol['nombres']} {sol['apellidos']}", sol["tipo_documento"])
...
def solicitudes_emitir(sid):
    ...
    if not sol:
        ...
        flash("Solicitud no encontrada", "error")
        return redirect(url_for("solicitudes"))
    horas = str(sol["horas_totales"] or "").strip()
    if not horas:
        conn.close()
    ...
    try:
        ...
        reemplazos = {
            "{{NOMBRE_COMPLETO}}": f"{sol['nombres']} {sol['apellidos']}",
            "{{DNI}}": str(sol["documento"] or ""),
            "{{UNIVERSIDAD}}": str(sol["universidad"] or ""),
            "{{FACULTAD}}": str(sol["facultad"] or ""),
            "{{CARRERA}}": str(sol["carrera"] or ""),
            "{{CODIGO}}": str(sol["codigo_alumno"] or ""),
            "{{FECHA_INICIO}}": formatear_fecha_latam(sol["fecha_inicio"]),
            "{{FECHA_FIN}}": formatear_fecha_latam(sol["fecha_fin"]),
            "{{CARGO}}": str(sol["cargo"] or "ASISTENTE").upper(),
            "{{HORAS_TOTALES}}": str(horas),
            "{{ACTIVIDADES}}": str(sol["actividades"] or ""),
        ...
        nombre_archivo = f"{tipo_doc}_{sol['documento']}_{uuid.uuid4().hex[:6]}.docx"
    ...
    codigo_doc = f"{tipo_doc}-{sol['documento']}-{datetime.now().strftime('%y%m%d')}"
    conn.execute("""
        UPDATE solicitudes
    ...
    add_historial(sid, sol["documento"] or "", session.get("nombre", "USUARIO"), "EMISION", "Documento generado y EMITIDO", f"{sol['nombres']} {sol['apellidos']}", sol["tipo_documento"])

    try:
        conn_my = db_mysql()
        with conn_my.cursor() as cur:
            nombre_completo = f"{sol['nombres']} {sol['apellidos']}"
            cur.execute("SELECT id FROM reportes WHERE solicitud_id = %s", (sid,))
            existe = cur.fetchone()
            if existe:
                cur.execute("""
                    UPDATE reportes
                    ...
                    WHERE solicitud_id = %s
                """, (
                    codigo_doc, nombre_completo, sol['documento'], tipo_doc,
                    ahora(), session.get("nombre", "USUARIO"), ruta_bd, sid
                ))
            else:
                cur.execute("""
                    INSERT INTO reportes
                        (solicitud_id, codigo_documento, nombre_completo, documento,
                         tipo_documento, estado, fecha_emision, emitido_por, ruta_pdf)
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)
                """, (
                    sid, codigo_doc, nombre_completo, sol['documento'], tipo_doc,
        ...
        print(f"ERROR DETALLADO REPORTES: {repr(e)}")
...
def solicitudes_anular(sid):
    ...
    conn.commit()
    conn.close()
    add_historial(sid, sol["documento"] or "", session.get("nombre", "USUARIO"), "ANULADO", "Solicitud ANULADA", f"{sol['nombres']} {sol['apellidos']}", sol["tipo_documento"])
    try:
        conn_my = db_mysql()
        with conn_my.cursor() as cur:
...
def plantillas():
    conn = db_mysql()
    with conn.cursor() as cur:
        cur.execute("SELECT * FROM plantillas ORDER BY actualizado_en DESC, id DESC")
        rows = cur.fetchall()
        cur.execute("SELECT COUNT(*) AS n FROM plantillas")
        total_row = cur.fetchone()
        total = total_row["n"] if total_row else 0
    conn.close()
    return render_template("plantillas.html", active="plantillas", plantillas=rows, total_plantillas=total)
...
def plantillas_toggle(pid):
    ...
    with conn.cursor() as cur:
        ...
        cur.execute("UPDATE plantillas SET activo = %s, actualizado_en = %s WHERE id = %s", (nuevo, ahora(), pid))
    conn.commit()
    conn.close()
    flash("Estado actualizado", "success")
    return redirect(url_for("plantillas"))
...
def reportes():
    ...
    where = ["estado IN ('EMITIDO', 'ANULADO')"]
    params = []
    ...
    sql = """
        SELECT
            ...
            ruta_pdf
        FROM reportes
        WHERE """ + " AND ".join(where) + " ORDER BY fecha_emision DESC LIMIT 500"
...
def configuracion():
    ...
    conn_lite.close()
    return render_template("configuracion.html", active="configuracion", usuario=u, config=config)
...
def configuracion_cambiar_password():
    ...
    conn = db_mysql()
    with conn.cursor() as cur:
        cur.execute("UPDATE usuarios SET password_hash = %s WHERE id = %s", (generate_password_hash(nueva), session["user_id"]))
        conn.commit()
    conn.close()
...
def ver_pdf(doc_id):
    conn = db()
    sol = get_solicitud_por_id(conn, doc_id)
    conn.close()
    if not sol or not sol["ruta_pdf"]:
        flash("Documento no encontrado", "error")
        return redirect(url_for("reportes"))
    ruta_abs = (BASE_DIR / sol["ruta_pdf"]).resolve()
...
def descargar_doc(doc_id):
    ...
    if not sol or not sol["ruta_pdf"]:
        ...
        return redirect(url_for("reportes"))
    ruta_abs = (BASE_DIR / sol["ruta_pdf"]).resolve()
    nombre_descarga = Path(sol["ruta_pdf"]).name
